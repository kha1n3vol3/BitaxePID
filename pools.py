...
def get_fastest_pools(yaml_file: str = "pools.yaml", 
        ...
        for pool in pools:
            ...
            try:
                ...
                if minutes_since_test > latency_expiry_minutes:
                    print(f"Latency data expired for {pool['endpoint']} "
    ...
    valid_pools = [pool for pool in pools if pool.get('latency', float('inf')) != float('inf')]
    ...
    sorted_pools[0]['user'] = stratum_user if stratum_user is not None else default_stratum_user
    sorted_pools[1]['user'] = fallback_stratum_user if fallback_stratum_user is not None else default_fallback_user
