...
# Ensure the ./pools/ directory exists
...
def parse_endpoint(endpoint_str: str) -> tuple[str, int]:
    ...
    Args:
        endpoint_str: Pool endpoint (e.g., 'stratum+tcp://stratum.solomining.io:7777').
    Returns:
...
def save_tdigest(hostname: str, port: int, tdigest: TDigest) -> None:
    ...
    Args:
        hostname: Pool hostname.
        port: Pool port.
        tdigest: TDigest object to save.
    """
    filename = f"./pools/{hostname}-{port}.tdigest.json"
    try:
...
def measure_single_latency(hostname: str, port: int, timeout: float = 5.0) -> float:
    ...
    Returns:
        Latency in milliseconds, or infinity if unreachable.
    """
...
def update_pools_yaml(endpoint: str, median: float) -> None:
    ...
    Args:
        endpoint: Pool endpoint string.
        median: Median latency to write.
    """
    try:
...
def measure_pools(yaml_file: str = "pools.yaml") -> List[Dict[str, Any]]:
    ...
    Args:
        yaml_file: Path to the YAML file.
    Returns:
        Updated list of pool dictionaries.
    """
...
def get_fastest_pools(
    ...
    Args:
        yaml_file: Path to the YAML file.
        stratum_user: Stratum user for primary pool.
        fallback_stratum_user: Stratum user for backup pool.
        user_yaml: Path to user YAML file.
        force_measure: Force new latency measurements.
        latency_expiry_minutes: Expiry time for latency data.
    Returns:
        List of up to two fastest pool dictionaries.
    """
    pools = load_pools(yaml_file)
    ...
    if stratum_user is None or fallback_stratum_user is None:
        try:
            ...
            default_fallback_user = user_config.get('fallbackStratumUser', default_stratum_user)
        except:
    ...
    for i, pool in enumerate(sorted_pools):
        print(f"{'Primary' if i == 0 else 'Backup'} pool: "
